...
<?php
	...
	$sql=$conn_simrs->query("
        select 
            ...
            round(sum(saldo) + sum(penerimaan) - sum(pengeluaran),2) saldoAkhir
        from (
            select
                concat(keu_saldo_awal.thn,'-',keu_saldo_awal.bln,'-01') as tgl,
				...
				<table class="table table-bordered table-striped jambo_table bulk_action dt-head-center" id="datatable-buttons"> 
						...
						<tbody>
							...
							<tr  class="bodylist" valign="top" bgcolor="<?php echo warna($i);?>">
								...
								<td><?php echo bulan($bs->bulan); ?></td>
								<td align="right"><?php if($bs->saldoAwal==0){echo '';}else{echo rp($bs->saldoAwal);} ?></td>
								...
								<td align="right"><?php if($bs->saldoAkhir==0){echo '';}else{echo rp($bs->saldoAkhir);} ?></td>
							</tr>
							<?php
								$i++; 
								}
							...
							<tr style="background-color: #FFFFFF">
								<td align="right" colspan="2">&nbsp;<?php echo rp($rs->awalx); ?></td>
								<td align="right">&nbsp;</td>
