...
<?php
	...
	$sql=$conn->query("select notrans,tanggal,kode50,keterangan,awal,sum(masuk) as masukx,sum(keluar) as keluarx from(
						   ...
						   and date(npkls_heder.tglpencairan)>='".$tgl."' and date(npkls_heder.tglpencairan)<='".$tglx."'  and npkls_heder.nopencairan<>''
						   group by npkls_heder.nonpk,npdls_rinci.nonpdls
							union all
							select spjpanjar_heder.nospjpanjar as notrans,spjpanjar_heder.tglspjpanjar as tanggal,spjpanjar_rinci.koderek50 as kode50,spjpanjar_rinci.rincianbelanja50 as keterangan,
							'' as awal,sum(spjpanjar_rinci.jumlahbelanjapanjar) as masuk,sum(spjpanjar_rinci.jumlahbelanjapanjar) as keluar 
							from spjpanjar_heder,spjpanjar_rinci
							where spjpanjar_heder.nospjpanjar=spjpanjar_rinci.nospjpanjar and 
							date(spjpanjar_heder.tglspjpanjar)>='".$tgl."' and date(spjpanjar_heder.tglspjpanjar)<='".$tglx."' group by spjpanjar_heder.nospjpanjar
	) as wew group by notrans,keterangan order by tanggal");
	$i=1;
?>
    ...
    <div class="x_panel">
      <div class="x_title">
        <center>BADAN LAYANAN UMUM DAERAH <?php echo  $rs1->nama;?></br>
				LAPORAN BUKU BANTU PAJAK BENDAHARA PENGELUARAN </br>
