...
<?php
	...
	if($_SESSION["anggaran_koderuangan"] == ''){
		$sql=$conn->query("select * from mappingpptkkegiatan where namapptk like '%".$usulantxt."%' and tahun='".$_SESSION["anggaran_tahun"]."' order by kodepptk");
	}else{
		$sql=$conn->query("select * from mappingpptkkegiatan where namapptk like '%".$usulantxt."%' and tahun='".$_SESSION["anggaran_tahun"]."'
		and kodebidang='".$_SESSION["anggaran_koderuangan"]."' order by kodepptk");
	}
...
<table class="table table-hover table-bordered table table-striped" id="dataTables-example">
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<input type="button" value="PILIH" onclick="pilihpptk('<?php echo $rs->kodepptk;?>','<?php echo $rs->namapptk; ?>','<?php echo $rs->kodekegiatan; ?>',
				'<?php echo $rs->kegiatan; ?>','<?php echo $rs->kodebidang; ?>','<?php echo $rs->bidang; ?>')">
			</td>
		</tr>
		<?php $i++; } ?>
	</tbody>
