...
async function gridrinci(notrans){ 
	...
	await $.get("trans_/penyesuaianperioritas/gridrinci.php",{notrans:notrans},
		function(result){ 
...
function simpanpenentuanprioritas(){ 
	...
	if(kodebidang==''){
		...
		koderek50:koderek50,uraianrek50:uraianrek50,jumlahacc:jumlahacc,nousulan:nousulan,satuan:satuan},
			function(result){ 
...
function hapus_rinci(id,notrans,nousulan,usulan){  
	swal({
	  ...
	  showLoaderOnConfirm: true
			...
			if(dismiss==true){
				$.get("trans_/penyesuaianperioritas/hapus_rinci.php",{id:id,notrans:notrans,nousulan:nousulan,usulan:usulan},
					function(result){ 
						var update = new Array();
						update = result.split('|');
						if(result.indexOf('|' != -1)) { 
...
function hapusHeader(notrans){
	if(confirm("apakah yakin data akan dihapus?")){
		$.get("trans_/penyesuaianperioritas/hapustransaksi.php",{notrans:notrans},function(result){ 
			if(result=="OK"){
				datapenentuanperioritas();
			}
			else{
...
function kunci(notrans){
	jfloading("sub_konten");
	$.get("trans_/penyesuaianperioritas/kunci.php",{notrans:notrans},function(result){ 
			var update = new Array();
			update = result.split('|');
			if(result.indexOf('|' != -1)) { 
				if(update[0]=="OK"){  
...
function bukakunci(notrans){
	jfloading("sub_konten");
	$.get("trans_/penyesuaianperioritas/bukakunci.php",{notrans:notrans},function(result){ 
			var update = new Array();
			update = result.split('|');
			if(result.indexOf('|' != -1)) { 
				if(update[0]=="OK"){  
