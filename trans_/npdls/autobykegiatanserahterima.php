...
<?php
	...
	$sql=$conn->query("select serahterima_heder.nokontrak as nokontrak,serahterima_heder.kodepihakketiga as kodepihakketiga,serahterima_heder.namaperusahaan as perusahaan,
						...
						serahterima_heder.kodepptk='".$_GET['kodepptk']."' and serahterima_heder.kegiatan like '%".$_REQUEST['query']."%' ");
	$data=array();
	$data['query']='Usulan';
	if ($sql && $sql->num_rows){
		while($rs = $sql->fetch_object()){
			$data['suggestions'][] = array(
				'value' => htmlentities($rs->kegiatan).'||'.htmlentities($rs->kegiatanblud).'||'.htmlentities($rs->perusahaan).'||'.htmlentities($rs->nokontrak),
