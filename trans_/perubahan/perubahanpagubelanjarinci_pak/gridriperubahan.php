...
<?php
	$sql=$conn->query("select id,notrans,nousulan,usulan,volume,satuan,harga,nilai,flagberubah from(
							   ...
							   notrans='".$_GET['notrans']."'
							   union all
							   select idpp,notrans,nousulan,usulan,volumebaru as volume,satuan,hargabaru as harga,totalbaru as nilai,'wew' as flagberubah 
							   from perubahanrincianbelanja_pak
							   where statusperubahan='1' and statusperubahan_pak_x='' and notrans='".$_GET['notrans']."'
...
<table class="table table-hover table-bordered table table-striped" id="dataTables-example">
		...
		<tbody>
			...
			<td><?php if($rs->flagberubah == "wew"){ echo "SUDAH P.A.K";}else{ echo $rs->flagberubah;} ?></td>
				<td>
