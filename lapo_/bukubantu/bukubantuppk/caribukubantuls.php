...
<?php
	...
	$sql=$conn->query("select notrans,tanggal,kode50,keterangan,awal,sum(masuk) as masukx,sum(keluar) as keluarx,urut from(
		   select noSpm as notrans,tglSpm as tanggal,'' as kode50,'' as keterangan,'' as awal,'' as masuk,jumlahspp as keluar,2 as urut 
		   from transSpm
		   where date(tglSpm)>='".$tgl."' and date(tglSpm)<='".$tglx."'
		   union all
		   select sppgu_heder.nosppgu as notrans,sppgu_heder.tglsppgu as tanggal,'' as kode50,sppgu_rinci.kegiatanblud as keterangan,'' as awal,'' as masuk,
		   sppgu_rinci.nilai as keluar,2 urut
