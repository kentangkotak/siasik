...
<?php
	...
	$sql=$conn->query("select npdpanjar_heder.nonpdpanjar as nonpdpanjar,npdpanjar_heder.tglnpdpanjar as tglnpdpanjar,
						...
						npdpanjar_heder.kegiatanblud as kegiatanblud,round(sum(npdpanjar_rinci.total),2) as total
						...
						and npdpanjar_heder.kegiatanblud like '%".$_REQUEST['query']."%' and npdpanjar_heder.verif=1
		...
		if ($sql && $sql->num_rows){
			while($rs = $sql->fetch_object()){
				$data['suggestions'][] = array(
					'value' => htmlentities($rs->kegiatanblud).'|'.$rs->nonpdpanjar ,
