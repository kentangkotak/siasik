...
<table class="table table-hover table-bordered table table-striped" id="dataTables-example">
	...
	<tbody>
		...
		<tr>
			...
			<td><input type="button" value="PILIH" onclick="pilihitembelanjanonfarmasi('<?php echo str_replace(array("\n", "\r"), '', $rs->itembelanja);?>',
			'<?php echo rpz($rs->volume);?>','<?php echo $rs->satuan; ?>',
			'<?php echo rpz($rs->harga); ?>','<?php echo rpz($rs->total); ?>','<?php echo $rs->notrans;?>','<?php echo $rs->nousulan;?>','<?php echo $rs->idpp;?>','<?php echo rpz($rs->total);?>','<?php echo rpz($rs->sisa);?>',
			'<?php echo $rs->koderek108;?>','<?php echo str_replace(array("\n", "\r"), '', $rs->uraian108);?>');",'2'></td>
		</tr>
		<?php $i++; } ?>
	</tbody>
</table>	
