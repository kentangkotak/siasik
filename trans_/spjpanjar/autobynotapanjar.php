...
<?php
	...
	$sql=$conn->query("select notapanjar_rinci.nonotapanjar as nonotapanjar,notapanjar_heder.kodepptk as kodepptk,notapanjar_heder.pptk as pptk,notapanjar_heder.program as program,
						...
						where notapanjar_rinci.nonotapanjar=notapanjar_heder.nonotapanjar and notapanjar_heder.nonotapanjar like '%".$_REQUEST['query']."%' group by notapanjar_rinci.nonotapanjar");
	$data=array();
	$data['query']='Usulan';
	if ($sql && $sql->num_rows){
