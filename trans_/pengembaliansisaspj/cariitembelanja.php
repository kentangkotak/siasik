	...
	if(isset($_GET['usulantxt']))
		...
		$rscari=$sqlcari->fetch_object();
			$sql=$conn->query("select * from (
								...
								nousulan,idpp,round(awal) as awal,round(sum(total)) as total,round(sum(beli)) as beli,round(sum(total-beli)) as sisasaldo from(
													...
													npdpanjar_rinci.nopp as nopp,npdpanjar_rinci.nousulan as nousulan,npdpanjar_rinci.idpp as idpp,npdpanjar_rinci.total as awal,
													npdpanjar_rinci.totalpermintaanpanjar as total,'' as beli
													...
													'' as rincianbelanja50,'' as nopp,'' as nousulan,spjpanjar_rinci.iditembelanjanpd as idpp,'' as awal,
													'' as total,spjpanjar_rinci.jumlahbelanjapanjar as beli
...
<table class="table table-hover table-bordered table table-striped" id="dataTables-example">
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<input type="button" value="PILIH" onclick="pilihitembelanja('<?php echo $rs->itembelanja;?>','<?php echo $rs->volume; ?>',
				...
				'<?php echo $rs->koderek50; ?>','<?php echo $rs->rincianbelanja50; ?>','<?php echo rpz($rs->sisasaldo); ?>','<?php echo rpz($rs->awal); ?>','<?php echo rpz($rs->beli); ?>')">
			</td>
		</tr>
		<?php $i++; } ?>
	</tbody>
