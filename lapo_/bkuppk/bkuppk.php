...
<?php
	...
	if($thn == 2022){
		$sql=$conn->query("select notrans,tanggal,kode50,keterangan,awal,sum(masuk) as masukx,sum(keluar) as keluarx,urut from(
			   ...
			   from saldoBankBk
			   where date(saldoBankBk.tglOpname)>='2022-03-31' and date(saldoBankBk.tglOpname)<='".$tglx."'
			   UNION all
			   select idTrans as notrans,date(tgltrans) as tanggal,'' as kode50,ket as keterangan,'' as awal,nilai as masuk,'' as keluar,1 as urut 
			   from t_terima_ppk
			   where date(tgltrans)>='2022-03-31' and date(tgltrans)<='".$tglx."'
			   union all
			   select noSpm as notrans,date(tglSpm) as tanggal,'' as kode50,uraian as keterangan,'' as awal,'' as masuk,jumlahspp as keluar,3 as urut 
			   from transSpm 
			   where date(tglSpm)>='2022-03-31' and date(tglSpm)<='".$tglx."'
			   UNION all
			   ...
			   and date(npkls_heder.tglpindahbuku)>='2022-03-31' and date(npkls_heder.tglpindahbuku)<='".$tglx."'  and npkls_heder.nopencairan<>''
			   group by npkls_heder.nonpk,npdls_rinci.nonpdls
			) as wew group by notrans,keterangan order by tanggal,urut");
	}else{
		$sql=$conn->query("sselect notrans,tanggal,kode50,keterangan,awal,sum(masuk) as masukx,sum(keluar) as keluarx,urut from(
			   select '' as notrans,saldoBankBk.tglOpname as tanggal,'' as kode50,'SALDO AWAL' as keterangan,saldoBankBk.nominal as awal,'' as masuk,'' as keluar,1 as urut 
			   from saldoBankBk
