		...
		$tglspm=in_tanggal("/",trim($_GET['tglspm']));
			...
			if($_GET['nospm']==''){
				...
				if($jml>0){ 
					...
					'".trim($_GET['tgltrans'])."','".trim($_GET['nip'])."','".trim($_GET['bendaharapengeluaran'])."','".$jumlahspp."','".trim($_GET['namabank'])."',
					'".trim($_GET['norekening'])."','".trim($_GET['uraianpekerjaan'])."','".$_SESSION["anggaran_kodeuser"]."','".$_SESSION["anggaran_username"]."',
					'".date('Y-m-d H:i:s')."')");
					
					$conn->query("update transsppup set buktiCreateSpm=1 where nosppup='".trim($_GET['nosppup'])."'");
					echo "OK|".$kode;
			}
