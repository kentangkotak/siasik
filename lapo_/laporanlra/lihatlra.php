				...
				<table class="table table-bordered table-striped jambo_table bulk_action dt-head-center" id="dataTables-examplex"> 
						...
						<tbody>
							<tr>
								...
								<td align="right" nowrap='nowrap'><?php echo rpzx($rs2->anggaran); ?></td>
								<td align="right" nowrap='nowrap'><?php echo rpzx($penerimaan); ?></td>
								<td align="right" nowrap='nowrap'><?php $selisih=$rs2->anggaran-$penerimaan; echo rpzx($selisih); ?></td>
								<td align="right" nowrap='nowrap'><?php if($rs2->anggaran == 0){ echo 0;}else{$selisihpersen=$penerimaan/$rs2->anggaran*100;echo round($selisihpersen,2);}?>%</td>
							</tr>
							<?php
							...
							<tr valign="top";>
								...
								<td align="right" nowrap="nowrap"><b><?php echo rpzx($anggaranTotal); ?></b></td>
								<td align="right" nowrap="nowrap"><b><?php echo rpzx($penerimaanTotal); ?></b></td>
								<td align="right" nowrap="nowrap"><b><?php echo rpzx($selisihTotal); ?></b></td>
								<td align="right" nowrap="nowrap"><b><?php echo round($selisihpersenTotal,2); ?>%</b></td>
							</tr>
								<?php
								$sql2x=$conn->query("select kode,uraian,sum(anggaran) as anggaranx,sum(realisasi)-sum(kurangi) as realisasix from(
															   ...
															   uraian,'' as anggaran,'' as realisasi,'' as kurangi 
															   from  akun50_miroring where akun=5 and kelompok is null
															   union all
															   ...
															   from contrapost
															   where tglcontrapost >='".$tgl."' and tglcontrapost <='".$tglx."'
													) as xxx");
								...
								<tr valign="top">
									...
									<td align="right" nowrap="nowrap"><b><?php echo rpzx($rs2x->anggaranx); ?></b></td>
									<td align="right" nowrap="nowrap"><b><?php echo rpzx($rs2x->realisasix); ?></b></td>
									<td align="right" nowrap="nowrap"><b><?php $selisih=$rs2x->anggaranx-$rs2x->realisasix; echo rpzx($selisih); ?></b></td>
									<td align="right" nowrap="nowrap"><b><?php $selisihpersen=$rs2x->realisasix/$rs2x->anggaranx*100; echo round($selisihpersen,2); ?>%</b></td>
								</tr>
								<?php
								$sql3x=$conn->query("select kode,uraian,sum(anggaran) as anggaranx,sum(realisasi)-sum(kurangi) as realisasix from(
													   select concat_ws('.',akun50_miroring.akun,akun50_miroring.kelompok,akun50_miroring.jenis,akun50_miroring.objectx,akun50_miroring.rincian,akun50_miroring.subrincian) as kode,
													   uraian,'' as anggaran,'' as realisasi,'' as kurangi 
													   from  akun50_miroring 
													   where akun=5 and jenis is null and kelompok is not null 
													   union all
								...
								<tr valign="top">
									...
									<td align="right" nowrap="nowrap"><b><?php echo rpzx($rs3x->anggaranx); ?></b></td>
									<td align="right" nowrap="nowrap"><b><?php echo rpzx($rs3x->realisasix); ?></b></td>
									<td align="right" nowrap="nowrap"><b><?php $selisih=$rs3x->anggaranx-$rs3x->realisasix; echo rp($selisih); ?></b></td>
									<td align="right" nowrap="nowrap"><b><?php $selisihpersen=($rs3x->realisasix/$rs3x->anggaranx)*100; echo round($selisihpersen,2) ;?>%</b></td>
								</tr>
										<?php
										$sql4x=$conn->query("select kode,uraian,sum(anggaran) as anggaranx,sum(realisasi)-sum(kurangi) as realisasix from(
															   ...
															   uraian,'' as anggaran,'' as realisasi,'' as kurangi
															   from  akun50_miroring 
															   where akun=5 and jenis is not null and objectx is null
															   union all
															   ...
															   where spjpanjar_rinci.nospjpanjar=spjpanjar_heder.nospjpanjar and spjpanjar_heder.verif=1
													           and spjpanjar_heder.tglspjpanjar >='".$tgl."' and spjpanjar_heder.tglspjpanjar <='".$tglx."'
															   and SUBSTRING_INDEX(spjpanjar_rinci.koderek50,'.',2)='".$rs3x->kode."' 
															   group by SUBSTRING_INDEX(spjpanjar_rinci.koderek50,'.',3)
															   union all
										...
										<tr valign="top">
											...
											<td align="right" nowrap="nowrap"><b><?php echo rpzx($rs4x->anggaranx); ?></b></td>
											<td align="right" nowrap="nowrap"><b><?php echo rpzx($rs4x->realisasix); ?></b></td>
											<td align="right" nowrap="nowrap"><b><?php $selisih=$rs4x->anggaranx-$rs4x->realisasix; echo rp($selisih); ?></b></td>
											<td align="right" nowrap="nowrap"><b><?php $selisihpersen=($rs4x->realisasix/$rs4x->anggaranx)*100;  echo round($selisihpersen,2) ;?>%</b></td>
										</tr>
											<?php
											...
											<tr>
												...
												<td align="right" nowrap="nowrap"><?php echo rpzx($rs5x->anggaranx); ?></td>
												<td align="right" nowrap="nowrap"><?php echo rpzx($rs5x->realisasix); ?></td>
												<td align="right" nowrap="nowrap"><?php $selisih=$rs5x->anggaranx-$rs5x->realisasix; echo rpzx($selisih); ?></td>
												<td align="right" nowrap="nowrap"><?php $selisihpersen=($rs5x->realisasix/$rs5x->anggaranx)*100;  echo round($selisihpersen,2) ;?>%</td>
											</tr>
											<?php
												$sql6x=$conn->query("select kode,uraian,sum(anggaran) as anggaranx,sum(realisasi)-sum(kurangi) as realisasix from(
																	   ...
																	   from npkls_heder,npkls_rinci,npdls_rinci
																		where npkls_heder.nopencairan=npkls_rinci.nopencairan 
																		and npkls_heder.tglpencairan >='".$tgl."' and npkls_heder.tglpencairan <='".$tglx."' 
																		and SUBSTRING_INDEX(npdls_rinci.koderek50,'.',4)='".$rs5x->kode."' 
																		and npdls_rinci.nonpdls=npkls_rinci.nonpdls
																		group by npdls_rinci.koderek50,npkls_heder.nopencairan,npdls_rinci.nonpdls
																	   union all
								...
								<tr class="p-3 mb-2 bg-danger text-white" valign="top";>
									<td colspan="2" align="right"><b>JUMLAH</b></td>
									<td align="right" nowrap="nowcrap"><b><?php echo rpzx($anggaranTotal); ?></b></td>
									<td align="right" nowrap="nowcrap"><b><?php echo rpzx($rs2x->realisasix); ?></b></td>
									<td align="right" nowrap="nowcrap"><b><?php echo rpzx($selisihTotal=$anggaranTotal-$rs2x->realisasix); ?><b></td>
									<td align="right" nowrap="nowcrap"><b><?php $selisihpersen=$rs2x->realisasix/$rs2x->anggaranx*100; echo round($selisihpersen,2); ?>%</b></td>
								</tr>
						</tbody>
				</table>
				</br>
