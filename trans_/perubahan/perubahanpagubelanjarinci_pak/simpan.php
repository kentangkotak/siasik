...
if($_GET['status_x'] == ''){
	...
	$nilai=$harga*$volume;
	$hargabaru= str_replace(',','',$_GET['harga']);
	$volumebaru= str_replace(',','',$_GET['volume']);
	...
	if($perubahanx > $pagu){
		...
		$conn->query("insert into perubahanrincianbelanja_pak(noperubahan,notrans,tglperubahan,usulan,volume,harga,nilai,volumebaru,hargabaru,totalbaru,selisih,
	...
	if($_GET['nousulan'] == ''){
		$conn->query("update perubahanrincianbelanja_pak set statusperubahan='2' where notrans='".trim($_GET['notrans'])."' and idpp='".trim($_GET['idpp'])."'");
		$sqlperubahan=$conn->query("select sum(total) as subtotal from(
									select sum(penyesesuaianperioritas_rinci.nilai) as total 
									from penyesesuaianperioritas_rinci,penyesesuaianperioritas_heder 
									where penyesesuaianperioritas_rinci.notrans=penyesesuaianperioritas_heder.notrans and 
		...
		$perubahanx=$perubahan+$totalbaru;
		if($perubahanx > $pagu){
			$conn->query("update perubahanrincianbelanja_pak set statusperubahan='1' where notrans='".trim($_GET['notrans'])."' and idpp='".trim($_GET['idpp'])."'");
		...
		$conn->query("update perubahanrincianbelanja_pak set statusperubahan='2' where notrans='".trim($_GET['notrans'])."' and idpp='".trim($_GET['idpp'])."'");
		$conn->query("update penyesesuaianperioritas_rinci set statusperubahan_pak='1' where notrans='".trim($_GET['notrans'])."' and idpp='".trim($_GET['idpp'])."'");
		$conn->query("update perubahanrincianbelanja set statusperubahan_pak='1' where notrans='".trim($_GET['notrans'])."' and idpp='".trim($_GET['idpp'])."'");
		$sqlperubahan=$conn->query("select sum(total) as subtotal from(
									select sum(penyesesuaianperioritas_rinci.nilai) as total 
									from penyesesuaianperioritas_rinci,penyesesuaianperioritas_heder 
									where penyesesuaianperioritas_rinci.notrans=penyesesuaianperioritas_heder.notrans and 
		...
		if($perubahanx > $pagu){
			$conn->query("update perubahanrincianbelanja_pak set statusperubahan='1' where notrans='".trim($_GET['notrans'])."' and idpp='".trim($_GET['idpp'])."'");
			$conn->query("update penyesesuaianperioritas_rinci set statusperubahan_pak='' where notrans='".trim($_GET['notrans'])."' and idpp='".trim($_GET['idpp'])."'");
			$conn->query("update perubahanrincianbelanja set statusperubahan_pak='' where notrans='".trim($_GET['notrans'])."' and idpp='".trim($_GET['idpp'])."'");
			$sqlperubahan2=$conn->query("select sum(total) as subtotal from(
									select sum(penyesesuaianperioritas_rinci.nilai) as total 
									from penyesesuaianperioritas_rinci,penyesesuaianperioritas_heder 
									where penyesesuaianperioritas_rinci.notrans=penyesesuaianperioritas_heder.notrans and 
	...
	if($perubahanx > $pagu){
		...
		echo "OK|".$kode."|5|".$perubahan."|".$totalbaru."|".$perubahanx."|".$pagu;
	}			
}
?>
