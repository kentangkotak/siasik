...
<?php
	...
	$sqlperubahan=$conn->query("
        select 
            sum(perubahanpagubelanjarinci.nilai) total
        from 
            perubahanpagubelanjarinci
        where
            perubahanpagubelanjarinci.kodekegiatan='".$_GET["kodekegiatan"]."'
            and year(perubahanpagubelanjarinci.tglperubahan)='".$_SESSION["anggaran_tahun"]."'
    ");
