...
<?php
	...
	if($_GET['notrans']==''){
		...
		$conn->query("insert into usulanHonor_r(notrans,keterangan,volume,harga,nilai,tglEntry,userEntry,satuan,bidangPengusul) values('".$kode."','".trim($_GET['keterangan'])."','".trim($volume)."',
		'".$harga."','".$nilai."','".date('Y-m-d H:i:s')."','".$_SESSION["anggaran_kodeuser"]."','".trim($_GET['satuan'])."','".trim($_GET['bidangPengusul'])."')");
