...
<?php
    ...
    $sqlUsulan=$conn->query("
        select 
            sum(usulanHonor_r_pak.nilai) total
        from 
            usulanHonor_h_pak, 
            usulanHonor_r_pak 
        where
            usulanHonor_h_pak.notrans=usulanHonor_r_pak.notrans
            and usulanHonor_h_pak.kodeKegiatan='".$_GET["kodekegiatan"]."'
            and year(usulanHonor_h_pak.tglTransaksi)='".$_SESSION["anggaran_tahun"]."'
    ");
