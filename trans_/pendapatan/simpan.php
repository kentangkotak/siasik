...
if ($_GET['notrans'] == '') {
	...
	$conn->query("insert into anggaran_pendapatan(notrans,bidang,koderekeningblud,uraian_rekening,nilai,tahun,tgl_entry,user_entry,map79,kode79,uraian79) values(
	...
	$conn->query("insert into t_tampung_pendapatan(notrans,pagu,tahun,koderekeningblud,kode79,uraian79) values(
			'" . $kode . "','" . $nominal . "',
			'" . $_SESSION["anggaran_tahun"] . "','" . trim($_GET['koderekeningblud']) . "','" . trim($_GET['kode79']) . "','" . trim($_GET['uraian79']) . "')");
	echo "OK|" . $kode;
} else {
	$kode = $_GET['notrans'];
	$conn->query("update anggaran_pendapatan set nilai='" . $nominal . "' where notrans='" . $kode . "'");
	$conn->query("update t_tampung_pendapatan set pagu='" . $nominal . "' where notrans='" . $kode . "'");
