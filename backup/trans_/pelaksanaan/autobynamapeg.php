...
<?php
	...
	$sql=$conn->query("select pegawai.nip as nip,pegawai.nama as nama,pegawai.bagian as koderuangan,m_bagian.namabagian as ruangan,pegawai.kategoripegawai as kategoripegawai,
						...
						and pegawai.pendidikan=m_pendidikan.kode and pegawai.golruang=m_golruang.kode_gol and pegawai.flag=m_jenispegawai.kode_jenis and pegawai.nama like '%".$_GET['query']."%' order by pegawai.nip ");
	$data=array();
	$data['query']='Pegawai';
	if ($sql && $sql->num_rows){
		while($rs = $sql->fetch_object()){
			$data['suggestions'][] = array(
				'value' => $rs->nama .' || '.$rs->nip ,
				'data' => $rs->nama,
				'nip' => $rs->nip,
